...
## [_escape-codegen-v0.2.0] - 2026-04-25

This release renames the codegen crate and ships the matching codegen-base
breaking change in lockstep:

| Crate                  | New version | Previous published |
| ---------------------- | ----------- | ------------------ |
| `v_escape-codegen-base`| 0.2.0       | 0.1.0              |
| `v_escape-codegen`     | 0.2.0       | 0.1.9              |

### Changed

- Rename the codegen crate from `v_escape_codegen` to `v_escape-codegen` so
  every workspace crate uses the same dashed naming convention. The
  hardcoded `v_escape_codegen@<version>` header in autogenerated `lib.rs`
  files is now derived from `CARGO_PKG_NAME`, so it stays in sync if the
...
- `v_escape-codegen-base::generate` now returns
  ...
  in their own documentation. `v_escape-codegen` 0.2.0 uses this to build
