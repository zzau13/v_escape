...
fn tests() {
    ...
    assert_eq!(
        VHtmlescape::from([short, escapes, short].join("").as_ref()).to_string(),
        [short, escaped, short].join("")
    );
    assert_eq!(
        VHtmlescape::from([escapes, short].join("").as_ref()).to_string(),
        [escaped, short].join("")
    );
    assert_eq!(
        VHtmlescape::from(["f", escapes, short].join("").as_ref()).to_string(),
        ["f", escaped, short].join("")
    );
    assert_eq!(
        VHtmlescape::from(["f", escapes].join("").as_ref()).to_string(),
        ["f", escaped].join("")
    );
    assert_eq!(
        VHtmlescape::from(["fo", escapes].join("").as_ref()).to_string(),
        ["fo", escaped].join("")
    );
    assert_eq!(
        VHtmlescape::from(["fo", escapes, "b"].join("").as_ref()).to_string(),
        ["fo", escaped, "b"].join("")
    );
    assert_eq!(
        VHtmlescape::from(escapes.repeat(2).as_ref()).to_string(),
        escaped.repeat(2)
    );
    assert_eq!(
        VHtmlescape::from(escapes.repeat(3).as_ref()).to_string(),
        escaped.repeat(3)
    );
    assert_eq!(
        VHtmlescape::from(["f", &escapes.repeat(2)].join("").as_ref()).to_string(),
        ["f", &escaped.repeat(2)].join("")
    );
    assert_eq!(
        VHtmlescape::from(["fo", &escapes.repeat(2)].join("").as_ref()).to_string(),
        ["fo", &escaped.repeat(2)].join("")
    );
    assert_eq!(
        VHtmlescape::from(["fo", &escapes.repeat(2), "bar"].join("").as_ref()).to_string(),
        ["fo", &escaped.repeat(2), "bar"].join("")
    );
    assert_eq!(
        VHtmlescape::from(["fo", &escapes.repeat(3), "bar"].join("").as_ref()).to_string(),
        ["fo", &escaped.repeat(3), "bar"].join("")
    );
    assert_eq!(
        VHtmlescape::from([&escapes.repeat(3), "bar"].join("").as_ref()).to_string(),
        [&escaped.repeat(3), "bar"].join("")
    );
    assert_eq!(
        VHtmlescape::from([short, &escapes.repeat(3), "bar"].join("").as_ref()).to_string(),
        [short, &escaped.repeat(3), "bar"].join("")
    );
    assert_eq!(
        VHtmlescape::from([short, &escapes.repeat(5), "bar"].join("").as_ref()).to_string(),
    ...
    assert_eq!(
        VHtmlescape::from([utf8, escapes, short].join("").as_ref()).to_string(),
        [utf8, escaped, short].join("")
    );
    assert_eq!(
        VHtmlescape::from([utf8, escapes, utf8].join("").as_ref()).to_string(),
        [utf8, escaped, utf8].join("")
    );
    assert_eq!(
        VHtmlescape::from([&utf8.repeat(124), escapes, utf8].join("").as_ref()).to_string(),
