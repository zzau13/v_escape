...
#[macro_export]
/// Macro `loop_m256_64` is the main loop that searches in byte slice with a bit mask
/// using `avx2` optimizations. Differs from `loop_m256_128` in the fact that it process
/// 64 bytes  every loop.
///
/// ## The following macros must be defined
///
        ...
        // set of VECTOR_SIZE elements and processed
            ...
            if LOOP_SIZE <= $len {
                while $ptr <= end_ptr.sub(LOOP_SIZE) {
                    ...
                    // Using function `_mm256_load_si256` for faster behavior on aligned bytes.
                    // Getting 2 sets of length `VECTOR_SIZE` each (`LOOP_SIZE=2*VECTOR_SIZE`)
                    let cmp_a = {
                        let a = _mm256_load_si256($ptr as *const __m256i);
                        masking!(a)
                    };
                    ...
                    // mask for the 2 sets of `VECTOR_SIZE` elements each, and make the pointer
                    // point to the next `LOOP_SIZE` elements
            ...
            // When the rest of string has a length greater then `VECTOR_SIZE`
            // but less than `LOOP_SIZE`, we process it `VECTOR_SIZE` bytes at
            // a time until there are left less then `VECTOR_SIZE` elements
            while $ptr <= end_ptr.sub(VECTOR_SIZE) {
                debug_assert_eq!(0, ($ptr as usize) % VECTOR_SIZE);
